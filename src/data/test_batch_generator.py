...
from batch_generators import generate_sum_input_batch, generate_sum_output_batch
...
def _test_input_batch_shape():
    input_batch = generate_sum_input_batch(batch_size=300)
    assert(input_batch.shape == (300, 60))


def _test_output_batch_shape():
    ones_matrix = np.ones((10, 60))
    output_batch = generate_sum_output_batch(ones_matrix)
